...
The scores are agent editorial checks, not customer research. Publication uses a separate directory containing only the 13 public website files; private comparison and research files are excluded. The public GitHub README now uses the same mission and qualified release claims. Published at https://0ba398e1.aether-many.pages.dev and both custom domains. Live HTML matches the approved content after accounting for the host-injected Cloudflare beacon; CSS and JavaScript match exactly. Both domains, deployment, assets, sitemap, and robots return 200; missing pages and the private review path return 404. Cloudflare’s managed robots additions retain the source Allow and sitemap entries. The live GitHub action reaches the revised project mission.
